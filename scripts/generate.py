...
def _literal_safe(value):
    """Coerce a value into JSON-ish Python primitives so `repr()` produces a
    rule_registry.py that imports with no dependencies.

    PyYAML parses an unquoted `reviewed: 2026-08-06` into a datetime.date; its
    repr is `datetime.date(2026, 8, 6)`, which the generated registry never
    imports — so the file the adapter loads would raise NameError. Dates become
    ISO strings; everything else that is already a primitive passes through.
    """
    if isinstance(value, dict):
        return {k: _literal_safe(v) for k, v in value.items()}
    if isinstance(value, (list, tuple)):
        return [_literal_safe(v) for v in value]
...
def gen_rule_registry(pairs) -> str:
    ...
    for bm, rule in pairs:
        ...
        lines.append(f"    {_literal_safe(entry)!r},")
    lines.append("]\n")
...
def gen_symptoms_alerts(pairs) -> str:
    ...
    for bm, rule in pairs:
        ...
        # Hoisted: nesting a same-quote f-string inside an f-string is 3.12+
        # syntax and will not parse on the 3.11 MP-builder environment.
        rule_name = f"{rule['id']} - {rule['title']}"
